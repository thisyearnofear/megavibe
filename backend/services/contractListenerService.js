...
class ContractListenerService {
  ...
  setupEventListeners() {
    ...
    this.tippingContract.on('TipSent', async (sender, recipient, amount, message, timestamp, event) => {
      console.log(`New tip detected: ${sender} -> ${recipient}, ${ethers.utils.formatEther(amount)} MNT`);
    ...
    this.bountyContract.on('BountyCreated', async (bountyId, creator, title, description, amount, deadline, event) => {
      console.log(`New bounty created: ${bountyId} by ${creator}, ${ethers.utils.formatEther(amount)} MNT`);
    ...
    this.bountyContract.on('BountyClaimed', async (bountyId, claimer, contentUrl, event) => {
