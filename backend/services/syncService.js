...
class SyncService {
  ...
  async syncTips(fromBlock = 0) {
    ...
    try {
      ...
      const events = await this.tippingContract.queryFilter(filter, fromBlock, currentBlock);
      ...
      for (const event of events) {
        ...
        const existingTip = await Tip.findOne({ transactionHash: event.transactionHash });
  ...
  async syncBounties(fromBlock = 0) {
    ...
    try {
      ...
      const createdEvents = await this.bountyContract.queryFilter(createdFilter, fromBlock, currentBlock);
      
      console.log(`Found ${createdEvents.length} bounty creation events to sync`);
      
      // Process each creation event
      for (const event of createdEvents) {
        const [bountyId, creator, title, description, amount, deadline] = event.args;
        
        // Check if this bounty already exists in the database
        const existingBounty = await Bounty.findOne({ bountyId: bountyId.toString() });
      ...
      const claimedEvents = await this.bountyContract.queryFilter(claimedFilter, fromBlock, currentBlock);
      ...
      return { 
        synced: { created: createdEvents.length, claimed: claimedEvents.length }, 
