...
class EventScrapingOrchestrator extends EventEmitter {
  ...
  setupEventListeners() {
    ...
    services.forEach(service => {
      ...
      service.on('rateLimited', (limitInfo) => {
        this.emit('serviceRateLimited', { service: service.name, ...limitInfo });
      });
    });
  }
  ...
  setupScheduledScraping() {
    if (!process.env.SCRAPING_ENABLED || process.env.SCRAPING_ENABLED !== 'true') {
  ...
  async scrapeWebSources() {
    ...
    for (const source of webSources) {
      try {
        ...
        const events = await this.firecrawlService.parseEventData(scrapedData, source);
    ...
    for (const source of socialSources) {
      try {
        ...
        if (source.platform === 'twitter') {
          const events = await this.masaService.searchTwitterEvents(source.queries, {
            maxResults: 100,
  ...
  async processAndSaveEvents(events) {
    ...
    console.log(`📊 Processed ${events.length} events: ${savedEvents.length} saved, ${duplicateEvents.length} duplicates`);
    return savedEvents;
  }
  ...
  async createEvent(eventData, venue) {
    const event = new Event({
      ...
      endTime: eventData.date ? new Date(eventData.date.getTime() + 8 * 60 * 60 * 1000) : new Date(), // 8 hours later
      artists: [], // Will be populated later
      description: eventData.description || `${eventData.name} - Web3 Conference`,
      ticketInfo: {
        price: 0,
        currency: 'USD',
      ...
      status: eventData.date && eventData.date > new Date() ? 'scheduled' : 'ended',
