...
const snippetSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ['performance', 'reaction', 'interview', 'ambient', 'announcement', 'other'],
    default: 'performance'
  },
  mood: {
    type: String,
    enum: ['energetic', 'chill', 'emotional', 'funny', 'intense', 'inspiring']
  },
  tags: [String],
  privacy: {
    type: String,
...
snippetSchema.virtual('engagementRate').get(function() {
  ...
  const totalEngagements = this.stats.likes + 
    this.stats.shares + 
    this.stats.comments + 
    this.stats.saves;
