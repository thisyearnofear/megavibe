...
const getOrchestrator = () => {
  if (!scrapingOrchestrator) {
    ...
    scrapingOrchestrator.on("scrapingCompleted", (data) => {
      console.log(
        `✅ Scraping completed: ${data.eventsSaved} events saved in ${data.duration}ms`
      );
    });
...
router.post("/test/masa", async (req, res) => {
  try {
    ...
    if (platform === "twitter") {
      const result = await orchestrator.masaService.searchTwitterEvents(
        [query],
        {
          maxResults: 10,
        }
...
router.get("/config", async (req, res) => {
  try {
    const {
      SCRAPING_SOURCES,
      SCRAPING_CONFIG,
    } = require("../services/scraping/ScrapingConfig.cjs");
...
router.get("/health", async (req, res) => {
  try {
    ...
    const allHealthy = Object.values(health).every(
      (service) => service.status === "healthy" || !service.configured
    );
