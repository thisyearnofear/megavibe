...
async function testVenueEndpoints() {
  ...
  logTestResult(
    ...
    searchResult.success
      ? `Found ${searchResult.data?.data?.length || 0} venues`
  ...
  const nearbyResult = await makeRequest(
    "GET",
    "/api/venues/nearby?lat=40.7128&lng=-74.0060&radius=10"
  );
  logTestResult(
    "Nearby Venues",
    nearbyResult.success,
    nearbyResult.success
      ? `Found ${nearbyResult.data?.data?.length || 0} venues`
      : `Status: ${nearbyResult.status}`
  );

  // Test invalid venue ID (should return 404)
  const invalidVenueResult = await makeRequest(
    "GET",
    "/api/venues/invalid-id",
    null,
    400
  );
  logTestResult(
    ...
    invalidVenueResult.success
      ...
      : `Status: ${invalidVenueResult.status}`
  );
}
...
async function testTipEndpoints() {
  ...
  logTestResult(
    ...
    getTipsResult.success
      ? `Found ${getTipsResult.data?.data?.tips?.length || 0} tips`
      : `Status: ${getTipsResult.status}`
  );

  // Test get event tips
  const eventTipsResult = await makeRequest(
    "GET",
    `/api/tips/event/${TEST_EVENT_ID}?limit=5`
  );
  logTestResult(
    "Get Event Tips",
    eventTipsResult.success,
    eventTipsResult.success
      ? `Found ${eventTipsResult.data?.data?.tips?.length || 0} tips`
      : `Status: ${eventTipsResult.status}`
  );

  // Test get speaker earnings
  const earningsResult = await makeRequest(
    "GET",
    `/api/tips/speaker/${TEST_SPEAKER_ID}/earnings`
  );
  logTestResult(
  ...
  // Test live tip feed
  const liveFeedResult = await makeRequest(
    "GET",
    `/api/tips/event/${TEST_EVENT_ID}/live?limit=10`
  );
  logTestResult(
    "Live Tip Feed",
    liveFeedResult.success,
    liveFeedResult.success
      ? `Found ${liveFeedResult.data?.data?.tips?.length || 0} tips`
      : `Status: ${liveFeedResult.status}`
  );

  // Test create tip without auth (should fail)
  const createTipResult = await makeRequest(
    "POST",
    "/api/tips/create",
    {
...
async function testBountyEndpoints() {
  ...
  logTestResult(
    ...
    getBountiesResult.success
      ? `Found ${getBountiesResult.data?.data?.bounties?.length || 0} bounties`
      : `Status: ${getBountiesResult.status}`
  );

  // Test get event bounties
  const eventBountiesResult = await makeRequest(
    "GET",
    `/api/bounties/event/${TEST_EVENT_ID}?limit=5`
  );
  logTestResult(
    "Get Event Bounties",
    ...
    eventBountiesResult.success
      ? `Found ${
          eventBountiesResult.data?.data?.bounties?.length || 0
        } bounties`
      : `Status: ${eventBountiesResult.status}`
  );

  // Test get speaker bounties
  const speakerBountiesResult = await makeRequest(
    "GET",
    `/api/bounties/speaker/${TEST_SPEAKER_ID}?limit=5`
  );
  logTestResult(
    ...
    speakerBountiesResult.success
      ? `Found ${
          speakerBountiesResult.data?.data?.bounties?.length || 0
        } bounties`
      : `Status: ${speakerBountiesResult.status}`
  );

  // Test create bounty without auth (should fail)
  const createBountyResult = await makeRequest(
    "POST",
    "/api/bounties",
    {
      eventId: TEST_EVENT_ID,
  ...
  logTestResult(
    ...
    createBountyResult.success
      ? "Correctly rejected unauthorized request"
      : `Status: ${createBountyResult.status}`
  );

  // Test invalid bounty ID
  const invalidBountyResult = await makeRequest(
    "GET",
    "/api/bounties/invalid-id",
    null,
    400
  );
  logTestResult(
    ...
    invalidBountyResult.success
      ? "Correctly returned 400"
      : `Status: ${invalidBountyResult.status}`
  );
}
