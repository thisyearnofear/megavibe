...
async function preDeploymentChecks() {
  ...
  const privateKeySet = process.env.PRIVATE_KEY ||
...
async function verifyContracts() {
  ...
  try {
    ...
    log.info('You can verify contracts manually later using: npm run verify:manual');
  }
}
...
async function updateEnvironmentConfigs(deploymentInfo) {
  ...
  const frontendEnvPath = path.join(CONFIG.frontend.directory, '.env.production');
...
function generateDeploymentSummary(deploymentInfo) {
  ...
  console.log('\n🔗 EXPLORER LINKS:');
  console.log(`  Tipping: https://explorer.mantle.xyz/address/${contracts.tipping.address}`);
  console.log(`  Bounties: https://explorer.mantle.xyz/address/${contracts.bounties.address}`);
